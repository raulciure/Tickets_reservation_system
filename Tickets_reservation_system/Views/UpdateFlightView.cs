...
namespace Tickets_reservation_system.Views
{
    public partial class UpdateFlightView : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (!Regex.Match(textBox3.Text, "^[0-9]*$").Success)
            {
            ...
            else if (!Regex.Match(textBox5.Text, "^[0-9]*$").Success)
            {
                MessageBox.Show("ADD A VALID SEATS/CLASS NUMBER");
                textBox1.Focus();
                return;
            ...
            else if (!Regex.Match(textBox7.Text, "^[1-9]*$").Success)
            {
                MessageBox.Show("ADD A VALID RANGE");
                textBox1.Focus();
                return;
            }
            else if (!Regex.Match(textBox9.Text, "^[a-zA-Z]*$").Success)
            {
                MessageBox.Show("ADD A VALID AIRPORT NAME");
                textBox1.Focus();
                return;
            }
            else if (!Regex.Match(textBox10.Text, "^[a-zA-Z]*$").Success)
            {
                MessageBox.Show("ADD A VALID AIRPORT NAME");
                textBox1.Focus();
                return;
            }
            else if (!Regex.Match(textBox11.Text, "^[1-9]*$").Success)
            {
                MessageBox.Show("ADD A VALID TIME");
                textBox1.Focus();
                return;
            }
            else if (!Regex.Match(textBox12.Text, "^[1-9]*$").Success)
            {
